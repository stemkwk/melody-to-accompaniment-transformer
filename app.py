...
def _extract_cond_midi(src_midi: Path, cfg, cond_tracks, out_path: Path) -> Path:
    """Write a MIDI containing ONLY the conditioning (melody) track(s).

    The input preview and the mix must reflect exactly what the model
    conditions on — the melody — not the original accompaniment that ships
    inside a multi-track file (e.g. POP909's MELODY/BRIDGE/PIANO). We extract
    the cond track, re-emit it as a single melody track, and use that for
    preview, mixing AND generation so all three stay consistent.

    Fallback: if the source has no track matching ``cond_tracks`` (a generic
    single-instrument melody whose name isn't "MELODY" maps to "accompaniment"),
    every note is relabelled as the melody so generation still runs instead of
    raising "No melody notes on track 'melody'".
    """
    from dataclasses import replace
    from jam_transformer.utils.midi_io import midi_to_events, events_to_midi

    events, tempo = midi_to_events(src_midi, cfg.tokenizer)
    ...
    if not mel:
        logger.warning(
            f"'{target}' 트랙을 찾지 못했습니다 — 입력 전체를 멜로디로 간주합니다."
        )
...
def _run_simple(
    ...
    try:
        ...
        if midi_file and raw_audio:
            ...
            # 멀티트랙 MIDI라도 melody 트랙만 조건화하도록 추출(없으면 전체를 멜로디로 폴백).
            src_midi = Path(midi_file if isinstance(midi_file, str) else midi_file.name)
            melody_midi = _extract_cond_midi(src_midi, cfg, cond_tracks, out_dir / "input_cond.mid")
            ...
            # MIDI만: 조건화에 쓰는 melody 트랙만 추출해 미리듣기·믹싱·생성에 사용
            # → 입력 미리듣기 = 멜로디만, mixed.wav = 멜로디 + 생성반주 (원곡 반주 제거)
            src_midi = Path(midi_file if isinstance(midi_file, str) else midi_file.name)
            melody_midi = _extract_cond_midi(src_midi, cfg, cond_tracks, out_dir / "input_cond.mid")
