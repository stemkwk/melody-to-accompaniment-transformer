...
def _build_callbacks(config: AppConfig, has_logger: bool):
    ...
    callbacks: List[pl.Callback] = [
        ...
        ModelCheckpoint(
            ...
            save_top_k=getattr(t, "checkpoint_save_top_k", 1),
            monitor=t.checkpoint_monitor,
            mode=t.checkpoint_monitor_mode,
            save_last=False,
            # every_n_epochs>1 with save_top_k=-1 thins the archive to fit a small
            # volume while keeping full (resumable) checkpoints.
            every_n_epochs=getattr(t, "checkpoint_archive_every_n_epochs", 1),
            save_weights_only=getattr(t, "checkpoint_save_weights_only", False),
        ),
...
def train(
    ...
    if sample_weights is not None:
        ...
        # Size the epoch to the ACTIVE pool (weight > 0). When a source is fully
        # excluded (weight 0 — e.g. lakh in the POP909 fine-tune), num_samples =
        # len(train_idx) would still count those never-sampled chunks, bloating
        # the epoch (each active chunk drawn ~Ntotal/Nactive times) and wasting
        # GPU. With no zero weights this equals len(train_idx) (unchanged).
        n_active = int((subset_weights > 0).sum().item())
        num_samples = max(1, n_active)
        ...
        logger.info(
            ...
            f"epoch={num_samples} active / {len(train_idx)} total chunks"
        )
